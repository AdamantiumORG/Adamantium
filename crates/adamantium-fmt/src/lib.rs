...
mod tests {
    ...
    fn formatted_example_project_still_parses_and_is_stable() {
        let source = include_str!("../../../example-project/code/main.ad");
        let formatted = format(source);
        let tokens = adamantium_lexer::lex(&formatted).unwrap();
        adamantium_parser::parse_checked(&formatted, &tokens).unwrap();
