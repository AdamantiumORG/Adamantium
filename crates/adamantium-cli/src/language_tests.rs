...
use crate::{analyze, build, diagnostics};
...
pub fn run(root: &Path, verbose: bool) -> Result<ExitCode, String> {
    let root = root
        .canonicalize()
...
fn run_case(case: &Case) -> Result<(), String> {
    ...
    let (code, output) = if case.valid {
        match build(&project.0) {
            ...
            Err(error) => (1, diagnostics::render_errors(&error)),
        }
    } else {
        match analyze(&project.0) {
            Ok(_) => (0, String::new()),
            Err(error) => (1, diagnostics::render_errors(&error)),
        }
    };
